...
export class InsertCategoriaIntegracao{
  ...
  async   cadastrar(  paramPedido:param )   {
        ...
        let sql =
         `  
            insert into ${db_integracao_mobile}.categorias
            (
            codigo_sistema,
            codigo_mobile,
            ativo
            ...
            `   
             ...
             await conn_sistema.query(sql,  (err:any, result:any[] )=>{
                if (err) {
                  console.log(`erro ao tentar inserir categoria sincronizado no banco da api `,err)
                  reject(err);
                }else{ 
                  resolve(result)
                }
