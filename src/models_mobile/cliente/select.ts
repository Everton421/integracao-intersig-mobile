...
export class Select_clientes_mobile{
    ...
    async   buscaGeral(empresa:any, vendedor:any )   {
       ...
       let sql = ` select *,
            ...
            WHERE c.ativo = 'S' and 
                       ...
                       order by c.vendedor    `
            await conn_mobie.query(sql,  (err:any, result:Cliente[] )=>{
                if (err)  reject(err); 
                  resolve(result)
            })
         })
    ...
    async   buscaPorVendedor(empresa:any, vendedor:number )   {
        ...
        let sql = ` SELECT *,
           ...
           FROM ${empresa}.clientes WHERE vendedor = ?  `
            await conn_mobie.query(sql, [ vendedor], (err:any, result:Cliente[] )=>{
                if (err)  reject(err); 
                  resolve(result)
            })
         })
    ...
    async   buscaPorcodigo(empresa:any, codigo:number )   {
        ...
        FROM ${empresa}.clientes WHERE codigo = ?  `
            await conn_mobie.query(sql, [ codigo], (err:any, result:Cliente[] )=>{
                if (err)  reject(err); 
                  resolve(result)
            })
         })
    ...
    async   buscaPorCnpj(empresa:any, cnpj:any )   {
      ...
      FROM ${empresa}.clientes WHERE cnpj = ?  `
          await conn_mobie.query(sql, [ cnpj], (err:any, result:Cliente[] )=>{
              if (err)  reject(err); 
                resolve(result)
          })
       })
